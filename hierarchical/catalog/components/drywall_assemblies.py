...
class BaseDrywallAssembly(ParametricComponent):
    """Base class for wall drywall assemblies"""

    DRYWALL_CLASS = None  # To be set by subclasses
    ...
    def _calculate_drywall_layout(self) -> Tuple[int, int, List[Tuple[float, float]]]:
        """Calculate optimal drywall layout to minimize waste"""
    ...
    def _create_elements_from_frame(self, frame_studs: List[Element]) -> List[Element]:
        ...
        if orientation == 'vertical':
            ...
            while current_x < self.params['wall_length']:
                ...
                if sheet_width > 0.1:
                    # CREATE element
                    sheet = self.DRYWALL_CLASS(length=sheet_width)
            ...
            for row in range(rows_needed):
                ...
                while current_x < self.params['wall_length']:
                    ...
                    if sheet_length > 0.1:
                        # CREATE element
                        sheet = self.DRYWALL_CLASS(length=sheet_length)
                        sheet.name = f"{self.name}_sheet_{sheet_num}"
        ...
        if orientation == 'vertical':
            ...
            for row in range(rows_needed):
                ...
                while current_x < self.params['wall_length']:
                    ...
                    if sheet_width > 0.1:
                        # CREATE element
                        sheet = self.DRYWALL_CLASS(width=sheet_width, length=sheet_length)
                        sheet.name = f"{self.name}_sheet_{sheet_num}"
    ...
    def _create_elements_standard_layout(self) -> List[Element]:
        ...
        sheets_across, sheets_up, sheet_sizes = self._calculate_drywall_layout()
        ...
        for row in range(sheets_up):
            for col in range(sheets_across):
                ...
                sheet = self.DRYWALL_CLASS(length=sheet_length)  # X-axis dimension
...
class BasicDryWallAssembly_0_25(BaseDrywallAssembly):
    """Base class for drywall assemblies"""
    MATERIAL_TYPE = "Drywall - Regular"
    DRYWALL_CLASS = Drywall_Regular_0_25


class BasicDrywallAssembly_0_375(BaseDrywallAssembly):
    """Standard interior drywall assembly"""
    DRYWALL_CLASS = Drywall_Regular_0_375
    MATERIAL_TYPE = "Drywall - Regular"

class BasicDrywallAssembly_0_5(BaseDrywallAssembly):
    """Standard interior drywall assembly"""
    DRYWALL_CLASS = Drywall_Regular_0_5
    MATERIAL_TYPE = "Drywall - Regular"
...
if __name__ == "__main__":
    ...
    drywall_assembly_standard = BasicDryWallAssembly_0_25(
        wall_height=8.0,
        wall_length=16.0,
    ...
    elements_standard = drywall_assembly_standard.create_elements()
    ...
    drywall_assembly_alligned = BasicDryWallAssembly_0_25(
        wall_height=10.0,
        wall_length=16.0,
        installation_orientation='vertical',
        stagger_joints=True,
    ...
    elements_aligned = drywall_assembly_alligned.create_elements()
    ...
    try:
        ...
        plot_items([drywall_assembly_alligned, wall_frame])
    except ImportError:
